...
func markWirePoint(x int, y int, wireId int, circuitBoard map[CircuitPoint]map[int]bool, intersectionDistances []int) (map[CircuitPoint]map[int]bool, []int) {
	if circuitBoard[CircuitPoint{x, y}] == nil { // the wire point is a map to avoid marking intersections of a wire on itself
		circuitBoard[CircuitPoint{x, y}] = map[int]bool{}
	}
	circuitBoard[CircuitPoint{x, y}][wireId] = true
	for k := range circuitBoard[CircuitPoint{x, y}] {
		if (k == wireId) {
			continue
		}

		manhattenDistance := Abs(x) + Abs(y) // centralpoint is 0,0, manhatten distance = |x2-x1|+|y2-y1| where x1 and y1 are always 0
		intersectionDistances = append(intersectionDistances, manhattenDistance)
		break;
    }
	return circuitBoard, intersectionDistances
}

func calculateMinimumDistance(wires []([]string)) int {
	circuitBoard := make(map[CircuitPoint]map[int]bool)
	var intersectionDistances []int
	for wireId, paths := range wires {
		currentX := 0
		currentY := 0
		for _, instruction := range paths {
			direction := string(instruction[0])
			distance, _ := strconv.Atoi(instruction[1:]) // just returning int(instruction[1]) returns byte value of string. e.g. "R8"[1] == 54, not 8

			// fmt.Printf("currentX: %d currentY: %d", currentX, currentY)
			// fmt.Printf("direction: %s", direction)
			// fmt.Printf("distance: %d", distance)
			if (direction == "R") {
				for i := 0;  i < distance; i++ {
					currentX++
					circuitBoard, intersectionDistances = markWirePoint(currentX, currentY, wireId, circuitBoard, intersectionDistances)
				}
			}
