...
func markWirePoint(x int, y int, wireId int, circuitBoard map[CircuitPoint]map[int]int, intersectionDistances []int, cumulativeSteps int) (map[CircuitPoint]map[int]int, []int) {
	if circuitBoard[CircuitPoint{x, y}] == nil { // the wire point is a map to avoid marking intersections of a wire on itself
		circuitBoard[CircuitPoint{x, y}] = map[int]int{}
	}

	circuitBoard[CircuitPoint{x, y}][wireId] = cumulativeSteps
	for k, v := range circuitBoard[CircuitPoint{x, y}] {
		if (k == wireId) {
			continue
		}

		stepsFromCentralPoint := v + cumulativeSteps
		intersectionDistances = append(intersectionDistances, stepsFromCentralPoint)
		break;
    }
...
func calculateMinimumDistance(wires []([]string)) int {
	circuitBoard := make(map[CircuitPoint]map[int]int)
		...
		for _, instruction := range paths {
			...
			if (direction == "R") {
				for i := 0;  i < distance; i++ {
					currentX++
					cumulativeSteps++
					circuitBoard, intersectionDistances = markWirePoint(currentX, currentY, wireId, circuitBoard, intersectionDistances, cumulativeSteps)
				}
			}
